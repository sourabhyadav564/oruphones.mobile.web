...
function TopDealNearBy({ selectedSearchCity, loading }) {
  ...
  useEffect(() => {
    if (!loading && selectedSearchCity != undefined) {
      bestDealNearByYou(selectedSearchCity, Cookies.get("info") || "Guest").then((response) => {
  ...
  return (
    <section className="px-3 text-sm text-gray-70">
      <h1 className="mt-3 mb-2  font-semibold text-base">Best Deals Near You ({selectedSearchCity})</h1>
