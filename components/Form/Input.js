  ...
  return (
    <div className={`outline outline-none relative w-full focus:outline-none ${className || ""}`}>
      ...
      >
        ...
        <input name={name} className={`py-3 block h-full w-full text-mx font-Roboto-Regular appearance-none ring-0 focus:ring-0 bg-transparent  text-[#2C2F45]  ${inputClass || ""}`} {...rest} />
      </div>
      <label
        htmlFor={name}
