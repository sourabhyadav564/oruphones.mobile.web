...
import { updateUserProfileDetails, getUserDetails, uploadUserProfilePic } from "api-call";
    ...
    <Fragment>
      ...
      <main>
        ...
        <section className="px-4 flex flex-col my-8 space-y-6">
          ...
          <Input disabled inputClass="text-black-21" defaultValue={user?.userdetails?.mobileNumber}>
            Mobile No
          </Input>
          <Input
            type="text"
          ...
          <Link href="/user/favourites">
            <a className="border border-primary py-3 text-sm text-primary uppercase rounded text-center">MY FAVORITES</a>
          </Link>
        </section>
      </main>
      <Footer />
...
const UserIcon = ({ onChange, img }) => (
  <div className={`p-1  relative ${img ? "bg-white rounded-full" : ""} `}>
    <input type="file" className="hidden" accept="image/*" id="photo" onChange={onChange} />
    {img ? (
      <div className="relative rounded-full flex justify-center items-center" style={{ width: 122, height: 122 }}>
        <Image src={img} layout="fill" className="rounded-full" objectFit="contain" />
      </div>
    ) : (
      <svg xmlns="http://www.w3.org/2000/svg" width="122" height="122" viewBox="0 0 122 122">
        <g transform="translate(-135 -143)">
          <circle cx="61" cy="61" r="61" transform="translate(135 143)" fill="#fff" />
          <path
